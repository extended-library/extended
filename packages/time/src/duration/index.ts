 ...
 */
function _isValid (object: any): boolean {
 ...
 */
function duration (duration?: any, defaultOrOptions?: string | number | DurationOptions, options?: DurationOptions): number {
  // process options --------------------------------------------------------------------------------------------------*
  // @ts-expect-error
  options = (options != null) || (typeof defaultOrOptions === 'object' && defaultOrOptions !== null
    ? defaultOrOptions
    : {}
  )
  ...
  // if (cached) {
  //   return (_resultCache as any)[input]
  // }
    ...
    if (duration.length > 0) {
      /*
        matches parts:
          ...
          - matches[3] - unit
      */
      for (let matches, matchedValue, matchedUnit; ((matches = _durationPattern.exec(duration)) != null);) {
        ...
        if (!isNaN(matchedValue)) {
          ...
          matchedUnit = matches[3] !== ''
            ? matches[3].toLowerCase()
            : 'ms'

          if (matchedUnit in _unitMap) {
            parsedDuration += matchedValue * (_unitMap as any)[matchedUnit]
          }
        }
      }
    }
  ...
  /* istanbul ignore else */
  // if (!cached) {
  //   (_resultCache as any)[input] = duration
  // }
...
//  */
// function createCustom (duration?: any, defaultOrOptions?: string | number | DurationOptions, options?: DurationOptions) {
//   // @ts-ignore
//   return function durationCustom (dur?, def?, opt?) {
//     // @ts-ignore
//     dur = typeof dur === 'string' || typeof dur === 'number' ? dur : 0
...
//     return __duration(dur, defaultDuration, opt)
//   }
// }
